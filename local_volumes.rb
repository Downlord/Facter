...
if Facter.value(:kernel) == 'Linux'
  ...
  Facter::Util::Resolution.exec('/bin/df -lP').each_line do |line|
    ...
    vmutex.synchronize { volumes << volume }

    # Skip header line ...
    next if not line.match(/\/dev\//)

    # Skip header line ...
    next if line.match(/\/dev\/shm/)

    # Parse line and retrieve name of the local volume ...
    disk_usage = line.split(' ')[4].strip

    # Parse line and retrieve name of the local volume ...
    disk = line.split(' ')[0].strip

    # Add single volume to the list ...
    dmutex.synchronize { disks << disk }
    dumutex.synchronize { disk_usages << disk_usage }

#   Facter.add("device_"+mydev) do
...
#   end
  ...
  Facter.add('local_disk_usages') do
    confine :kernel => :linux
    setcode { disk_usages.join(',') }
  end

  Facter.add('local_disk_max_usage') do
    confine :kernel => :linux
    setcode { disk_usages.max }
  end
