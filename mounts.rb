...
if Facter.value(:kernel) == 'Linux'
  ...
  Facter::Util::Resolution.exec('cat /proc/mounts 2> /dev/null').each_line do |line|
    ...
    #
    raw_device = row[0].strip.gsub('\\040', ' ')
    if raw_device.match(/by-uuid/)
      device = Facter::Util::Resolution.exec('readlink -f '+raw_device)
    else
      device = raw_device
    end
    mount  = row[1].strip.gsub('\\040', ' ')
    fstype  = row[2].strip.gsub('\\040', ' ')
    rawdevice = device

  Facter.add('mounted_'+rawdevice) do
    confine :kernel => :linux
    setcode { mount }
    #setcode { mount+','+fstype }
  end
    #
